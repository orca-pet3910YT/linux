...
static bool __read_mostly sysrq_always_enabled = 1;
...
static void emerg_sync_filesystems(u8 key) {
	// please don't be confused by this function name!
	// this function is a kernel panic MADE TO TRIGGER
	// a joke message
	rcu_read_unlock(); // release read lock - donno why but safety apparently
	panic("This PC is ass.\n"); // the panic
}
static const struct sysrq_key_op sysrq_emergsync_op = {
	.handler        = emerg_sync_filesystems, // the panic function
	.help_msg       = "emergsync(A)", // operation name, used in SysRq + H IIRC
	.action_msg     = "Trigger an emergency sync", // the name for our joke
	.enable_mask    = SYSRQ_ENABLE_BOOT, // permission level for rebooting. remember how this kernel is not for businesses?
};
...
static const struct sysrq_key_op sysrq_crash_op = {
	...
	.enable_mask	= SYSRQ_ENABLE_BOOT, //SYSRQ_ENABLE_DUMP,
};
